...
vec3 envMap(vec3 rd, vec3 sn) {
  ...
  rd.xy -= iTime * SPEED * .25;
  rd *= 3.;

  float c = n3D(rd) * .57 + n3D(rd * 2.) * .28 + n3D(rd * 4.) * .15; // Noise value.
  c = smoothstep(.4, 1., c); // Darken and add contast for more of a spotlight look.

  // Superdark metallic grey env, gold highlights
  vec3 col = mix(vec3(0.04, 0.04, 0.045), vec3(1.0, 0.55, 0.05), c * c);

  return mix(col, col * 0.7, sRd * .25 + .25);
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  // Surface color value.
  vec3 oC = vec3(0.04, 0.04, 0.045); // superdark metallic grey base

  if (fold.x > 0.) oC = vec3(0.06, 0.06, 0.065) * (c2 * .5 + .5); // superdark metallic grey for squiggle space

  if (fold.x < 0.05 && (fold.y) < 0.) oC = vec3(1.0, 0.55, 0.05) * (c2 * .25 + .75); // bright orange-gold borders
  else if (fold.x < 0.) oC = vec3(0.75, 0.42, 0.04) * c2; // darker orange-gold overlay
  ...
  vec3 col = oC * (diff + .5) + vec3(1.0, 0.55, 0.05) * spec * 2. + vec3(0.3, 0.15, 0.01) * fre;
