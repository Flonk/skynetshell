...
bool isSpecialHex(vec2 ce) {
  const float k3 = 1.732050807;
  ivec2 prismID = ivec2(round(ce.x), round(ce.y / k3));
...
float map(vec2 p, in float time)
{
  ...
  float h = kMaxH * (0.005 + 0.995 * f);
  if (isSpecialHex(p_orig)) {
    h *= 1.0 + 0.1 * sk_keypulse_envelope();
  }
  return h;
}
  ...
  out ivec2 outPrismID, out int outFaceID)
{
  ...
  {
    {
      res = vec4(t1.x, s1 * vec3(n1.x, 0, n1.y));
      outFaceID = (d1 < 0.0) ? -1 : 1;
    }
    if (t2.x > res.x) {
      res = vec4(t2.x, s2 * vec3(n2.x, 0, n2.y));
      outFaceID = (d2 < 0.0) ? -2 : 2;
    }
    if (t3.x > res.x) {
      res = vec4(t3.x, s3 * vec3(n3.x, 0, n3.y));
      outFaceID = (d3 < 0.0) ? -3 : 3;
    }
    if (t4.x > res.x) {
      res = vec4(t4.x, s4 * vec3(0.0, 1, 0));
      outFaceID = (d4 < 0.0) ? 4 : -4;
    }
...
vec3 render(in vec3 ro, in vec3 rd, in float time)
{
  ...
  if (t > 0.0)
  {
    ...
    vec3 mate = vec3(1.0);
    id = hash(id);
    mate *= 0.1 + 0.9 * float((id >> 13) & 3) / 3.0;
    id = hash(id);
    vec3 specialCol = vec3(0.0, 0.3, 0.8);
    specialCol = mix(specialCol, vec3(1.0, 0.5, 0.0), sk_load_envelope());
    specialCol = mix(specialCol, sk_fail_color, sk_fail_envelope());
    mate = (((id >> 8) & 15) == 0) ? specialCol : mate;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  for (int m = 0; m < AA; m++)
    for (int n = 0; n < AA; n++)
    {
      ...
      float time = iTime * SPEED - 0.5 * (1.0 / 24.0) * (float(m * AA + n) + d) / float(AA * AA);
      #else
      ...
      #if AA>1
      vec3 fp = ro + rd * 17.0;
      vec2 ra = texelFetch(iChannel1, (q + ivec2(13 * m, 31 * n)) & 511, 0).xy;
      ro.xy += 0.06 * sqrt(ra.x) * vec2(cos(6.2831 * ra.y), sin(6.2831 * ra.y));
      rd = normalize(fp - ro);
      #endif
