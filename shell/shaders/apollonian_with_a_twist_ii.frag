...
vec3 effect(vec2 p, vec2 q) {
  ...
  vec3 col = vec3(0.0);
  float bgBoost = 1.0 + 0.2 * clamp(sk_keypulse_envelope() + sk_load_envelope(), 0.0, 1.0);
  float bgScalar = mix(0., 1.0, diff) * 0.5 * mix(0.1, 1.0, ss) / (lightLen3 * lightLen3) * bgBoost;
  vec3 bgTint = mix(vec3(1.0), sk_fail_color, sk_fail_envelope() * 0.5);
  col += bgScalar * bgTint;
