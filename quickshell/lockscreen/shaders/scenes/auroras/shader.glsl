...
vec4 aurora(vec3 ro, vec3 rd)
{
  ...
  for (float i = 0.; i < 50.; i++)
  {
    ...
    float heightT = pow(i / 49.0, 0.4);
    vec3 loadTarget = mix(vec3(0.05, 0.9, 0.25), vec3(184.0 / 255.0, 22.0 / 255.0, 114.0 / 255.0) * 1.5, heightT) * rzt;
    col2.rgb = mix(col2.rgb, loadTarget, sk_load_envelope());
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  float aurBright = 1.0 + 0.15 * sk_keypulse_envelope();
  float aurFade = 1.0 - sk_fail_envelope();

  if (rd.y > 0.) {
    vec4 aur = smoothstep(0., 1.5, aurora(ro, rd)) * fade;
    aur.rgb *= aurBright * aurFade;
    aur.a *= aurFade;
    col += stars(rd);
